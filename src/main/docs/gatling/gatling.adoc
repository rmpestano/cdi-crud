...
TIP: Refer to http://gatling.io/docs/2.1.5/general/simulation_setup.html[gatling documentation] for more examples.
...
So at the end of these 3 steps we have 150 http requests.
...
IMPORTANT: I will run the simulation and the application (the one deployed on Wildfly) on the same machine. This is *NOT the ideal* cause both will compete for resources (CPU and memory) but is what I have at the moment and also simpler to show the concepts.
...
TODO link reports online (eg flickr or even embed on application or on github)
...
This section will guide you on how do you run the simulation examplified here.
...
----
. build the project, cd to to project root directory
+
----

run mvn clean package
----
. copy target/cdi-crud.war ro wildfly deployment directory
. start wildfly
. at project dir run the command:
+
----
mvn gatling:execute
----

reports are generated at target/gatling dir.

=== Running via docker
I've created a docker container so you can run this simulation easier

==== Pre requisites
You just need https://www.docker.com/[Docker] installed on your machine.
